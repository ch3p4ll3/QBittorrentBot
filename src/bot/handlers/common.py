...
async def list_active_torrents(
    ...
    categories_buttons = [
        InlineKeyboardButton(
            Translator.translate(Strings.ListFilterDownloading, user.locale, active='*' if status_filter == 'downloading' else ''),
            callback_data="by_status_list#downloading"
        ),
        InlineKeyboardButton(
            Translator.translate(Strings.ListFilterCompleted, user.locale, active='*' if status_filter == 'completed' else ''),
            callback_data="by_status_list#completed"
        ),
        InlineKeyboardButton(
            Translator.translate(Strings.ListFilterPaused, user.locale, active='*' if status_filter == 'paused' else ''),
    ...
    if not torrents:
        buttons.append([InlineKeyboardButton(Translator.translate(Strings.BackToMenu, user.locale), callback_data="menu")])
        try:
            await bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
    ...
    for torrent in torrents:
        if callback:
            buttons.append([InlineKeyboardButton(torrent.name, callback_data=f"{callback}#{torrent.hash}")])
        else:
            buttons.append([InlineKeyboardButton(torrent.name, callback_data=f"torrentInfo#{torrent.hash}")])
