...
def get_router():
    ...
    async def on_magnet(message: Message, user, redis: RedisWrapper, bot: Bot):
        if message.text.startswith("magnet:?xt"):
            ...
            await send_menu(bot, redis, message.id, message.from_user.id)
    ...
    async def on_torrent(message: Message, user, redis: RedisWrapper, bot: Bot):
    ...
    async def on_category_name(message: Message, redis: RedisWrapper, bot: Bot):
    ...
    async def on_category_directory(message: Message, action, redis: RedisWrapper, bot: Bot):
    ...
    async def on_edit_user(message: Message, user, action, redis: RedisWrapper, bot: Bot):
        data = action.split("#")[1]
    ...
    async def on_edit_client(message: Message, user, action, redis: RedisWrapper, bot: Bot):
        data = action.split("#")[1]
        ...
        try:
            ...
            setattr(Configs.client, field_to_edit, new_value)
            Configs.update_config(Settings)
            Configs.reload_config()
    ...
    @inject_user
    async def on_message(message: Message, redis: RedisWrapper, user: User, bot: Bot) -> None:
        action = await redis.get(f"action:{message.from_user.id}")

        if "magnet" in action:
            await on_magnet(message, user, redis, bot)

        elif "torrent" in action and message.document:
            await on_torrent(message, user, redis, bot)

        elif action == "category_name":
            await on_category_name(message, redis, bot)

        elif "category_dir" in action:
            await on_category_directory(message, action, redis, bot)

        elif "edit_user" in action:
            await on_edit_user(message, user, action, redis, bot)

        elif "edit_clt" in action:
            await on_edit_client(message, user, action, redis, bot)
        else:
            await message.reply(
